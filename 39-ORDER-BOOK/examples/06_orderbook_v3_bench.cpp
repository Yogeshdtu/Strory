// 06_orderbook_v3_bench.cpp
// ============================================================
// V3 (flat array + intrusive list + flat hash) ki per-operation LATENCY
// DISTRIBUTION -- (14-measuring-v3.md). SAME workload as 02/04.
// ============================================================
//   g++ -std=c++20 -O2 -Wall -Wextra 06_orderbook_v3_bench.cpp -o v3b && ./v3b
// ============================================================
...
int main() {
    ...
    const auto ops = generate_workload(N, /*seed=*/999);   // SAME seed as 02/04

    // max_orders generous rakha gaya -- N=200000 workload live-order-count
    // ko 150000 se kabhi upar nahi le jaata (V1/V2 pe verified ~92k peak);
    // agar arena bhar jaata, add() silently fail hoti aur book V1/V2 se
    // DIVERGE kar jaata -- isliye yeh sizing correctness ke liye zaroori
    // hai, sirf capacity-planning nahi.
    BookV3 book(/*center=*/10000, /*max_orders=*/150000);
    ...
    std::printf("=== BookV3 (flat array + intrusive list + flat hash) ===\n");
