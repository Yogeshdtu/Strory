...
| [✓] | Namespaces — nesting/aliases (01-cheatsheet), internal/external/**module** linkage, `static` at namespace scope vs anonymous namespace, `const`/`constexpr` default-internal, API hygiene (24/05) | 24 |
...
| [~] | Deducing `this` (C++23) — explicit object parameter, one member template covers const/`&`/`&&`, recursive lambdas; **fully explained lesson-level in `22/05`, compile-verification deferred** (repo default is `-std=c++20`) → tracked in `WHAT-I-STILL-NEED-TO-LEARN.md` SPECIALIZED list, not an open gap | 22 |
...
| [~] | `<coroutine>` — the **machinery is covered** (`coroutine_handle`, `promise_type`, `suspend_always`/`suspend_never`, hand-rolled `Generator<T>`, frame/HALO — `22/09`); only C++23 `std::generator` + executors (not in C++23) are out → SPECIALIZED list, not an open gap | 22 |
...
| Section | ✓ | ~ | ☐ | % done |
|---|---|---|---|---|
| Language fundamentals | 81 | 4 | 0 | ~95% |
| Standard library | 33 | 1 | 3 | ~89% |
| Memory + object model | 16 | 0 | 0 | ~97% |
| Concurrency | 21 | 0 | 0 | ~97% |
| Systems | 17 | 0 | 0 | ~100% (examples Linux-only) |
| Build + tooling | 9 | 2 | 4 | ~63% |
| Performance | 17 | 0 | 0 | ~100% |

**Overall: roughly 89% complete.**  Concurrency and Systems programming (Linux
internals + networking) are **done** at the lesson level. **Folders 31–36 (CPU
architecture, cache & memory performance, compiler optimization, assembly,
profiling & benchmarking, ultra-low-latency C++) are now all complete —
Batch 9 done, Batch 10 started.** Folder 33 lifted "Compiler optimizations" and
"SIMD / auto-vectorization"; folder 34 "Reading assembly"; folder 35
"Benchmarking methodology", "Percentiles / tail latency", "Profiling with
perf/VTune". **Folder 36 lifts the last four — "Allocation avoidance", "Memory /
object pools", "Ring buffers", "Batching & syscall avoidance" — to ✓, making
the entire Performance section complete (~100%).** Folder 36 is the *synthesis*:
folders 12–35's techniques applied to hot-path engineering, **every technique
paired with its hidden cost and a "when NOT to"** — pools / object pools /
arenas / PMR, cache locality, false sharing, branch-free (and when it *loses*),
virtual-dispatch elimination, `std::function` cost, ring buffers, batching,
syscall & page-fault avoidance, CPU pinning, cache warming, I-cache / hot-cold
split, zero-copy, compile-time dispatch, and lesson 24's honest trade-off
catalogue + the measure→profile→one-change→re-measure→explain process. Rule 2
fired 3× (all taught). **Remaining: the HFT domain track (folders 37–44) and the
wrap-up folders (45–49) — no C++/systems knowledge gaps.**
